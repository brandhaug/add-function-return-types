...
describe('incremental cache', (): void => {
	...
	it('writes a cache file after processing', async (): Promise<void> => {
		...
		const cache = JSON.parse(await fs.readFile(getCachePath(dir), 'utf-8'))
	...
	it('skips unchanged files on a second run', async (): Promise<void> => {
		...
		await run({ path: dir })
		const firstPass = await fs.readFile(filePath, 'utf-8')
		...
		await run({ path: dir })
		expect(await fs.readFile(filePath, 'utf-8')).toContain('): string {')
	})
	...
	it('invalidates the whole cache when options change', async (): Promise<void> => {
		...
		await run({ path: dir })
		const afterFirstRun = await fs.readFile(filePath, 'utf-8')
		...
		expect(await fs.readFile(filePath, 'utf-8')).toBe('const get = () => 42;\n')
	})
	...
	it('does not write or read the cache with useCache disabled', async (): Promise<void> => {
		...
		await expect(fs.readFile(getCachePath(dir), 'utf-8')).rejects.toThrow()

		// A cached run afterwards still works from scratch.
		await fs.writeFile(filePath, 'function greet() {\n  return 1;\n}\n')
		await run({ path: dir })
		expect(await fs.readFile(filePath, 'utf-8')).toContain('): number {')
	})
	...
	it('never writes a cache file during dry runs', async (): Promise<void> => {
		...
		expect(await fs.readFile(filePath, 'utf-8')).not.toContain(': string {')
	})
