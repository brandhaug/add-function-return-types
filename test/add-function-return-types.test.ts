...
describe.concurrent('add-function-return-types', (): void => {
  ...
  const runAddFunctionReturnTypes = async (overrides: Partial<Options> = {}): Promise<void> => {
...
`.trim()
    ...
    const updatedSource = await fs.readFile(filePath, 'utf-8')
    expect(updatedSource).toContain('const multiply = (a: number, b: number): number =>')
  })
...
`.trim()
    ...
    const updatedSource = await fs.readFile(filePath, 'utf-8')
    expect(updatedSource).toContain('async function fetchData(url: string): Promise<any> {')
  })
...
`.trim()
    ...
    const updatedSource = await fs.readFile(filePath, 'utf-8')
    expect(updatedSource).toContain('function logMessage(message: string): void {')
  })
...
`.trim()
    ...
    const updatedSource = await fs.readFile(filePath, 'utf-8')
    expect(updatedSource).toContain('function* idGenerator(): Generator<number, void, unknown> {')
  })
...
`.trim()
    ...
    const updatedSource = await fs.readFile(filePath, 'utf-8')
    expect(updatedSource).toContain('function toNumber(value: string | number): number {')
  })
...
`.trim()
    ...
    const updatedSource = await fs.readFile(filePath, 'utf-8')
    expect(updatedSource).toContain(`function greet(name: string = 'World'): string {`)
  })
...
`.trim()
    ...
    const updatedSource = await fs.readFile(filePath, 'utf-8')
    expect(updatedSource).toContain('function sum(...numbers: number[]): number {')
  })
...
`.trim()
    ...
    const updatedSource = await fs.readFile(filePath, 'utf-8')
    expect(updatedSource).toContain('const double = function(n: number): number {')
  })
...
`.trim()
    ...
    const updatedSource = await fs.readFile(filePath, 'utf-8')
    expect(updatedSource).toContain('function createAdder(a: number): (b: number) => number {')
  })
...
`.trim()
    ...
    const updatedSource = await fs.readFile(filePath, 'utf-8')
    expect(updatedSource).toContain('function toNumber(value: string): number | undefined {')
  })
...
`.trim()
    ...
    const updatedSource = await fs.readFile(filePath, 'utf-8')
    expect(updatedSource).toContain('function toNumber(value: string): number | null {')
  })
...
`.trim()
    ...
    const updatedSource = await fs.readFile(filePath, 'utf-8')
    expect(updatedSource).toContain('function firstItem(values: string[]): string | undefined {')
  })
...
`.trim()
    ...
    const updatedSource = await fs.readFile(filePath, 'utf-8')
    expect(updatedSource).toContain('const typedFunction: () => number = function() {')
    expect(updatedSource).toContain('const untypedFunction = function(): number {')
  })
...
`.trim()
    ...
    const updatedSource = await fs.readFile(filePath, 'utf-8')
    expect(updatedSource).toContain('const arrowVoid = () => void doSomething();')
...
`.trim()
    ...
    const updatedSource = await fs.readFile(filePath, 'utf-8')
    expect(updatedSource).toContain('const typedFunction: () => number = function(): number {')
  })
  ...
  it('ignores return type if overwrite is true and ignoreHigherOrderFunctions is true', async (): Promise<void> => {
    ...
    expect(updatedSource).toContain('function higherOrder(callback: () => number): number | null {')
  })
...
`.trim()
    ...
    const updatedSource = await fs.readFile(filePath, 'utf-8')
    expect(updatedSource).toContain('function getObject(): { foo: string; baz: number; } {')
...
`.trim()
    ...
    expect(updatedSource).toContain('async function getNormalType(): Promise<string> {')
  })
...
`.trim()
    ...
    const updatedSource = await fs.readFile(filePath, 'utf-8')
    expect(updatedSource).toContain('async function returnPromiseAny(): Promise<any> {')
    expect(updatedSource).toContain('async function inferredPromiseAny(): Promise<any> {')
  })
...
`.trim()
    ...
    const updatedSource = await fs.readFile(filePath, 'utf-8')
    expect(updatedSource).toContain('async function returnPromiseUnknown(): Promise<unknown> {')
    expect(updatedSource).toContain('async function inferredPromiseUnknown(): Promise<unknown> {')
  })
...
`.trim()
    ...
    const updatedSource = await fs.readFile(filePath, 'utf-8')
    expect(updatedSource).toContain('async function getPromiseObject(): Promise<{ foo: string; baz: number; }> {')
    expect(updatedSource).toContain('async function getNormalType(): Promise<string> {')
    expect(updatedSource).toContain('function getRegularObject(): { foo: string; } {')
  })
...
`.trim()
    ...
    expect(updatedSource).toContain('function getNormalRecord(): Record<string, string> {')
  })
...
`.trim()
    ...
    const updatedSource = await fs.readFile(filePath, 'utf-8')
    expect(updatedSource).toContain('function getRecordAny(): Record<string, any> {')
    expect(updatedSource).toContain('function getArrayUnknown(): unknown[] {')
    expect(updatedSource).toContain('function getArrayAnonymous(): { foo: string; baz: number; }[] {')
  })
