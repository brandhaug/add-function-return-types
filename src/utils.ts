 ...
 */
async function searchForGitFileRecursive(currentPath: string): Promise<string | null> {
  try {
...
export async function findRepoRoot(startPath: string): Promise<string> {
  ...
  console.info(`No .git directory found, using start path as root: "${startPath}"`)
  return startPath
}
 ...
 */
export async function findPackageJsonFiles(startPath: string): Promise<string[]> {
 ...
 */
export async function getDependencies(packageJsonPaths: string[]): Promise<string[]> {
  ...
  for (const packageJsonPath of packageJsonPaths) {
    try {
      ...
      console.info(`Processing ${depsCount} dependencies from "${packageJsonPath}"`)
