...
import { type Expression, ModuleKind, Node, Project, ScriptTarget, SyntaxKind, ts } from 'ts-morph'
...
export async function addFunctionReturnTypes(options: Options): Promise<void> {
  ...
  console.info('Processing complete after %d seconds', (endTime - startTime) / 1000)
}
 ...
 */
async function getAllTsAndTsxFiles(rootPath: string, options: Options): Promise<string[]> {
 ...
 */
async function processFile(project: Project, filePath: string, options: Options): Promise<string> {
  ...
  sourceFile.forEachDescendant((node): void => {
    try {
      ...
      const name = Node.isMethodDeclaration(node) || Node.isFunctionDeclaration(node) ? node.getName() : undefined
      ...
      if (options.ignoreExpressions && (Node.isFunctionExpression(node) || Node.isArrowFunction(node))) {
        return
      }

      // ignoreTypedFunctionExpressions: ignore function expressions with type annotations on the variable
      if (options.ignoreTypedFunctionExpressions && (Node.isFunctionExpression(node) || Node.isArrowFunction(node))) {
      ...
      if (options.ignoreFunctionsWithoutTypeParameters && node.getTypeParameters().length === 0) {
        return
      }
      ...
      if (options.ignoreHigherOrderFunctions) {
        ...
        if (expr && (Node.isFunctionExpression(expr) || Node.isArrowFunction(expr))) {
          return
        }
      }

      // ignoreConciseArrowFunctionExpressionsStartingWithVoid: ignore arrow functions starting with `void`
      if (options.ignoreConciseArrowFunctionExpressionsStartingWithVoid && Node.isArrowFunction(node)) {
        const body = node.getBody()
      ...
      if (body) {
        ...
        if (returnExpr && Node.isIdentifier(returnExpr)) {
          const param = node.getParameters().find((p): boolean => p.getName() === returnExpr.getText())
          if (param) {
